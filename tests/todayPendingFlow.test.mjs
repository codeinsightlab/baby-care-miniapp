...
assert.match(reminderPageSource, /displayState:\s*\{[\s\S]*babyId:\s*''[\s\S]*reminders:\s*\[\][\s\S]*loaded:\s*false/)
assert.match(reminderPageSource, /refreshingState:\s*\{[\s\S]*reminders:\s*false/)
assert.match(reminderPageSource, /hasReminderDisplayState\(\)[\s\S]*String\(this\.displayState\.babyId\) === String\(this\.currentBabyId\)/)
assert.match(reminderPageSource, /isInitialLoading\(\)[\s\S]*this\.refreshingState\.reminders && !this\.hasReminderDisplayState/)
assert.match(reminderPageSource, /isRefreshingReminders\(\)[\s\S]*this\.refreshingState\.reminders && this\.hasReminderDisplayState/)
assert.match(reminderPageSource, /reminderRequestSeq/)
assert.match(reminderPageSource, /requestSeq !== this\.reminderRequestSeq \|\| String\(targetBabyId\) !== String\(this\.currentBabyId\)/)
...
assert.match(reminderPageSource, /recentCompensationReminders\(\)[\s\S]*this\.reminders\.slice\(-RECENT_COMPENSATION_LIMIT\)/)
assert.match(reminderPageSource, /earlierCompensationReminders\(\)[\s\S]*this\.reminders\.slice\(0,\s*Math\.max\(0,\s*this\.reminders\.length - RECENT_COMPENSATION_LIMIT\)\)/)
assert.match(reminderPageSource, /<view class="reminder-flow-section">[\s\S]*<view v-else class="reminder-card-flow">\s*<reminder-card[\s\S]*v-for="item in recentCompensationReminders"/)
assert.match(reminderPageSource, /还有 \s*\$\{this\.earlierCompensationReminders\.length\} 条更早提醒/)
assert.match(reminderPageSource, /<view v-if="earlierExpanded" class="earlier-card-flow">[\s\S]*v-for="item in earlierCompensationReminders"[\s\S]*mode="compensation"/)
assert.match(reminderPageSource, /toggleEarlierReminders\(\)[\s\S]*this\.earlierExpanded = !this\.earlierExpanded/)
assert.match(reminderPageSource, /\.reminder-card-flow[\s\S]*gap:\s*26rpx/)
assert.match(reminderPageSource, /\.earlier-toggle[\s\S]*background:\s*rgba\(255,\s*255,\s*255,\s*0\.72\)/)
